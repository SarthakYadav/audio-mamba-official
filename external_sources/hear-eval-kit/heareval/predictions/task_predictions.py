...
class SplitMemmapDataset(Dataset):
    ...
    def __init__(
        ...
        if in_memory:
            ...
            print("Moving tensors to cuda")
            if "maestro" in str(embedding_path):
               print("Working with maestro, not moving to gpu")
            else:
...
def task_predictions(
    ...
    if deterministic:
        print(f"USING random seed {random_seed} for determinism")
        seed_everything(random_seed, workers=False)
